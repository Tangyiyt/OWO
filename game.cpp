...
void Game::showStatus(){
    cout<<"\n=====  狀態  ====="<<endl;
    cout<<player.getName()<<" HP: "<<player.getHp()<<" | 藥水: "<<player.getPotions()<<endl;
...
void Game::battle(){
    ...
    while(player.isAlive()&&enemy.isAlive()){
        ...
        if(choice == 1){
            cout<<player.getName()<<"攻擊"<<enemy.getName()<<"造成了"<<player.getAttack()<<"點傷害"<<endl;
        ...
        if(enemy.isAlive()){
            cout<<enemy.getName()<<"攻擊"<<player.getName()<<"造成了"<<enemy.getAttack()<<"點傷害"<<endl;
